...
"""}
    ...
    async with semaphore:
        ...
        st.warning(f"ページ {page_num} のチェックに失敗しました: {e}")
        return ""
    ...
    async with semaphore:
        ...
        for attempt in range(3):
            try:
                ...
                e = ex
                await asyncio.sleep(5)
        st.warning(f"フィルタリングチャンク {chunk_index} の処理に失敗しました: {e}")
        return [], []
...
if uploaded_file is not None:
    ...
    # Excel出力用（不要列削除）
    excel_filtered = filtered_results_df.drop(columns=["重要度", "自信度", "情報種別"], errors='ignore')
    excel_excluded = excluded_results_df.drop(columns=["重要度", "自信度", "情報種別"], errors='ignore')
    excel_all = all_df_results.drop(columns=["重要度", "自信度", "情報種別"], errors='ignore')
